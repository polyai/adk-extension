...
export function activate(context: vscode.ExtensionContext) {
	...
	const checkForUpdatesDisposable = vscode.commands.registerCommand(
		'adk-extension.checkForUpdates',
		async () => {
			await performUpdateCheck(context, true);
		}
	);
	...
	// Check for updates automatically on every activation
	checkForExtensionUpdates(context);
}

/**
 * Checks for extension updates from GitHub
 */
async function checkForExtensionUpdates(context: vscode.ExtensionContext): Promise<void> {
	console.log('[Update Checker] checkForExtensionUpdates called - checking on every activation');
	
	const lastCheck = getLastUpdateCheck(context);
	if (lastCheck) {
		const hoursSince = (Date.now() - lastCheck) / (1000 * 60 * 60);
		console.log(`[Update Checker] Last check was ${hoursSince.toFixed(2)} hours ago`);
	} else {
		console.log('[Update Checker] No previous check found');
	}

	console.log('[Update Checker] Proceeding with automatic update check');
	await performUpdateCheck(context, false);
}

/**
 * Performs the actual update check
 * @param context Extension context
 * @param showNoUpdateMessage Whether to show a message if no update is available (for manual checks)
 */
async function performUpdateCheck(
	context: vscode.ExtensionContext,
	showNoUpdateMessage: boolean = false
): Promise<void> {
	console.log('[Update Checker] performUpdateCheck called', { showNoUpdateMessage });
	
	try {
		// Get extension information
		const extension = vscode.extensions.getExtension('undefined_publisher.adk-extension');
		if (!extension) {
			console.error('[Update Checker] Extension not found');
			if (showNoUpdateMessage) {
				vscode.window.showErrorMessage('Could not find extension information');
			}
			return;
		}
		...
		console.log('[Update Checker] Extension info:', {
		...
		if (!repository || !repository.url) {
			console.error('[Update Checker] No repository URL found in package.json');
			if (showNoUpdateMessage) {
		...
		console.log('[Update Checker] Parsing repository URL:', repoUrl);
		...
		if (httpsMatch) {
			owner = httpsMatch[1];
			repo = httpsMatch[2];
			console.log('[Update Checker] Parsed repository:', { owner, repo });
		}

		if (!owner || !repo) {
			console.error('[Update Checker] Could not parse repository URL:', repoUrl);
			if (showNoUpdateMessage) {
				vscode.window.showErrorMessage(`Could not parse repository URL: ${repoUrl}`);
			}
			return;
		}
		...
		if (!githubToken) {
			console.error('[Update Checker] GITHUB_ACCESS_TOKEN environment variable not found');
			const errorMessage = 'GitHub access token not found. Please set the GITHUB_ACCESS_TOKEN environment variable to check for updates.';
			if (showNoUpdateMessage) {
				vscode.window.showErrorMessage(errorMessage);
			} else {
				console.error('[Update Checker]', errorMessage);
			}
			return;
		}

		if (showNoUpdateMessage) {
			console.log('[Update Checker] Manual update check initiated');
			await vscode.window.withProgress(
				...
				async () => {
					try {
						console.log('[Update Checker] Calling checkForUpdates...');
						const updateInfo = await checkForUpdates(owner!, repo!, currentVersion);
						// Update last check time
						await setLastUpdateCheck(context, Date.now());
						console.log('[Update Checker] Update check result:', JSON.stringify(updateInfo, null, 2));
						
						if (updateInfo && updateInfo.hasUpdate) {
							console.log('[Update Checker] Update available, showing notification');
							// Show notification
							await notifyUpdate(updateInfo, packageJson.displayName || 'ADK Extension', context);
						} else if (showNoUpdateMessage) {
							console.log('[Update Checker] No update available');
						...
						const errorMessage = error instanceof Error ? error.message : String(error);
						console.error('[Update Checker] Exception during update check:', error);
						vscode.window.showErrorMessage(`Failed to check for updates: ${errorMessage}`);
					}
				}
			);
		} else {
			// Automatic check (non-blocking, silent on error)
			console.log('[Update Checker] Automatic update check initiated');
			checkForUpdates(owner, repo, currentVersion)
				.then(async (updateInfo) => {
					// Update last check time
					await setLastUpdateCheck(context, Date.now());
					console.log('[Update Checker] Automatic check result:', JSON.stringify(updateInfo, null, 2));
					...
					if (updateInfo && updateInfo.hasUpdate) {
						...
						console.log('[Update Checker] No update available (automatic check)');
					}
				})
				.catch((error) => {
					// Silently fail - don't bother user with update check errors
					console.error('[Update Checker] Automatic update check failed:', error);
				});
		}
	} catch (error) {
		const errorMessage = error instanceof Error ? error.message : String(error);
		if (showNoUpdateMessage) {
			vscode.window.showErrorMessage(`Error checking for updates: ${errorMessage}`);
		}
		console.log('Error setting up update check:', error);
	}
}
